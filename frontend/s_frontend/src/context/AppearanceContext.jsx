...
function normalizeAppearance(appearance = {}) {
    return {
        ...
        lobbyCount: appearance.lobbyCount || 5,
        soundEnabled: appearance.soundEnabled ?? true,
        boardColor: normalizeBoardColor(appearance.boardColor)
    };
}
...
export function AppearanceProvider({ children }) {
    ...
    useEffect(() => {
        ...
        const persistedAppearance = normalizeAppearance(user?.appearance);
        const currentAppearance = normalizeAppearance({
            theme,
            lobbyCount,
            soundEnabled,
            boardColor
        ...
        if (JSON.stringify(persistedAppearance) === JSON.stringify(currentAppearance)) {
            return;
        }
        ...
        const timeout = setTimeout(async () => {
            ...
            try {
                await updateUser(userId, { appearance }, "registered");
